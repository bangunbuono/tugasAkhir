...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if($keterangan == "weekly"){
        $query = "SELECT bahan, sum(jumlah) as jumlah, satuan, DATE(tanggal_keluar) as tanggal_keluar
                FROM barang_keluar where bahan = '$bahan' AND 
                WEEK(tanggal_keluar,1) = $week AND user = '$user'
        ...
        if($check > 0){
            ...
            while($ambil = mysqli_fetch_object($execute)){
                ...
                $M["tanggal_keluar"] = $ambil -> tanggal_keluar;
        ...
        $query = "SELECT bahan, sum(jumlah) as jumlah, satuan, DATE(tanggal_keluar) as tanggal_keluar
                FROM barang_keluar where bahan = '$bahan' AND 
                MONTH(tanggal_keluar) = $month AND user = '$user'
                GROUP BY DAY(tanggal_keluar);";
        $execute = mysqli_query($connect, $query);
        $check = mysqli_affected_rows($connect);
        ...
        $query = "SELECT bahan, sum(jumlah) as jumlah, satuan, DATE(tanggal_keluar) as tanggal_keluar
                FROM barang_keluar where bahan = '$bahan' AND 
                YEAR(tanggal_keluar) = $year AND user = '$user'
                GROUP BY DAY(tanggal_keluar);";
        $execute = mysqli_query($connect, $query);
        $check = mysqli_affected_rows($connect);
